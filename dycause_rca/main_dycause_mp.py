    ...
    **kws,
        ...
        for x_i in range(len(data_head)):
            for y_i in range(len(data_head)):
                ...
                if array_results_YX is None or array_results_XY is None:
                    local_results["%s->%s" % (x_i, y_i)]["intervals"] = []
                    local_results["%s->%s" % (x_i, y_i)]["result_YX"] = None
                    local_results["%s->%s" % (x_i, y_i)]["result_XY"] = None
                else:
                    matrics = [array_results_YX, array_results_XY]
                    ordered_intervals = get_ordered_intervals(
                        matrics, significant_thres, list_segment_split
    ...
    for x_i in range(len(data_head)):
        for y_i in range(len(data_head)):
            ...
            histogram_sum[key] = overlay_counts.sum()
    # skip the \r print line
